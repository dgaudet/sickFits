...
class UserPermissions extends React.Component {
  ...
  state = {
    // we are using props here to seed the initial state, otherwise
    // you would not use props to set state here
              ...
              {possiblePermissions.map(perm => (
                <td key={perm}>
                  <label htmlFor={`${user.id}-permission-${perm}`}>
                    <input id={`${user.id}-permission-${perm}`}
                    type="checkbox" checked={this.state.permissions.includes(perm)}
                    value={perm}
